...
table_id = "buoyant-apogee-281013.FIN.GURU_DATA"
job_config = bigquery.QueryJobConfig()
# =========== Read the table =================
sql = f"""
SELECT *
FROM `buoyant-apogee-281013.FIN.GURU_DATA`
"""
sql_data = pandas_gbq.read_gbq(sql, project_id=project_id)
sql_data = sql_data.set_index('index')
...
params = pd.read_excel('PARAMS.xlsx').fillna('')[1:-1]
...
for list, index, exchange, exchange_yahoo, row, columns_first, columns_second in zip(LIST_list, index_list, \
    ...
    for ticker in tickers:
        ...
        with open(
                f'''C:/Users/Anton/Desktop/CODE_SMILE/BACKTESTING/BANKA/data_json_{ticker.replace(exchange, '')}.pickle''',
                # УКАЗАТЬ СВОЙ ПУТЬ !!!!
                'rb') as f:
            data_json = pickle.load(f)

        with open(
                f'''C:/Users/Anton/Desktop/CODE_SMILE/BACKTESTING/BANKA/data_json_keyratios_{ticker.replace(exchange, '')}.pickle''',
                # УКАЗАТЬ СВОЙ ПУТЬ !!!!
                'rb') as f:
        ...
        try:
            ...
            cashflow_df = pd.DataFrame(io['cashflow_statement']).set_index(date_list).replace('No Debt', 0).replace \
                ('At Loss', 0).replace('-', 0).replace('', 0).replace('N/A', 0).astype(float)
            valuation_ratios_df = pd.DataFrame(io['valuation_ratios']).set_index(date_list).replace('No Debt', 0).replace \
                ('At Loss', 0).replace('-', 0).replace('', 0).replace('N/A', 0).astype(float)
            ...
            common_size_ratios_df = pd.DataFrame(io['common_size_ratios']).set_index(date_list).replace('No Debt', 0) \
                .replace('At Loss', 0).replace('', 0).replace('-', 0).replace('N/A', 0).replace('Negative Tangible Equity',
                                                                                                0).astype(float)
               ...
               # returns calc
                ...
                for i_trend in range(len(income_df)):
                    try:
                        ...
                        # print(df2)
                    # try:
                        df1 = pd.concat([price_yahoo_main_full['Close'][ticker.replace(exchange, '') + exchange_yahoo], price_yahoo_main_full['Open'][ticker.replace(exchange, '') + exchange_yahoo],
                                         price_yahoo_main_full['High'][ticker.replace(exchange, '') + exchange_yahoo], price_yahoo_main_full['Low'][ticker.replace(exchange, '') + exchange_yahoo]],
                        ...
                        data = fl.swings(df, high='rebased_high', low='rebased_low', argrel_window=20)
                ...
                for mom_num in range(len(income_df)):
                    if list == 'Китай':
                        exchange = 'HKSE:0'
                    try:
                        prices = price_yahoo_main[ticker.replace(exchange, '') + exchange_yahoo][:period[mom_num].split('-')[0]].asfreq('BM')
...
# -------------------------------- РАСЧЕТНЫЙ ЦИКЛ ----------------------------------------
    ...
    try:
        with pd.ExcelWriter('PORTFOLIO PER YEAR/PORTFOLIO_XGBoost_100.xlsx', mode='a') as writer:
            portfolio_df_final.to_excel(writer, sheet_name=list)
    except:
        with pd.ExcelWriter('PORTFOLIO PER YEAR/PORTFOLIO_XGBoost_100.xlsx', mode='w') as writer:
            portfolio_df_final.to_excel(writer, sheet_name=list)
...
Returnez_finish[::-1].to_excel('ReturnS_XGBoost_100_EBITDA.xlsx')
