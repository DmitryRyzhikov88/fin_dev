...
for list, index, exchange, exchange_yahoo, row, columns_first, columns_second in zip(LIST_list, index_list, \
    ...
    for ticker in tickers:
        ...
        if check == 1:
            ...
            try:
                ...
                Data_for_Portfolio['EBITDA'] = income_df['EBITDA']
    ...
    print(Data_for_Portfolio_TOTAL)
#
# -------------------------------- РАСЧЕТНЫЙ ЦИКЛ ----------------------------------------
    if list == 'Китай':
        exchange = 'HKSE:0'
    ...
    for i in range(years_len):
        ...
        Data_for_Portfolio_master_filter.to_excel(f'Total_Score{list}.xlsx')
        ...
        # print(index)
        # print(yf.download(index)['Adj Close'].fillna(method='backfill')[str(int(start) + i)])
