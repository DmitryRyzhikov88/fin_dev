        ...
        if check == 1:
            ...
            # XGBoost =========================================================================================
               # returns calc
                ...
                X = DF_corr[
                    ['ROCE %', 'EV-to-EBITDA', 'YoY Rev. per Sh. Growth', 'Beneish M-Score', 'Interest Coverage', \
                     'Change In Other Working Capital', 'Other Income (Expense)', 'PE Ratio without NRI',
                     'Asset Turnover', 'EV-to-Revenue', 'Shares Buyback Ratio %', 'Scaled Net Operating Assets', 'Equity-to-Asset']]
...
# -------------------------------- РАСЧЕТНЫЙ ЦИКЛ ----------------------------------------
    ...
    for i in range(years_len):
        ...
        print('Год расчета портфеля: ', start)
...
#     # print(portfolio_df_final)
#
#     try:
#         with pd.ExcelWriter('PORTFOLIO PER YEAR/PORTFOLIO_XGBoost_(fixed_feach)_Valuation3.0_(normal).xlsx', mode='a') as writer:
#             portfolio_df_final.to_excel(writer, sheet_name=list)
#     except:
#         with pd.ExcelWriter('PORTFOLIO PER YEAR/PORTFOLIO_XGBoost_(fixed_feach)_Valuation3.0_(normal).xlsx', mode='w') as writer:
#             portfolio_df_final.to_excel(writer, sheet_name=list)
#
#
#     print('^'*50)
...
#
# Returnez_finish[::-1].to_excel('ReturnS_XGBoost_(fixed_feach)_Valuation3.0_(normal).xlsx')
#
#
