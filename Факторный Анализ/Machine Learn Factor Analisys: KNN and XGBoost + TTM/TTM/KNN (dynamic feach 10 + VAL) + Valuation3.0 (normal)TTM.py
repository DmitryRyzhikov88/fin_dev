    ...
    for ticker in tickers:
        ...
        if check == 1:
            ...
            # XGBoost =========================================================================================
                ...
                X = pd.concat([DF_corr[TOP_feach], Data_for_Portfolio['EBITDA/EV']], axis=1, ignore_index=False)
...
#
    ...
    try:
        with pd.ExcelWriter('KNN (dynamic feach 10 + VAL) + Valuation3.0 (normal)TTM.xlsx', mode='a') as writer:
            portfolio_df_final.to_excel(writer, sheet_name=list)
    except:
        with pd.ExcelWriter('KNN (dynamic feach 10 + VAL) + Valuation3.0 (normal)TTM.xlsx', mode='w') as writer:
            portfolio_df_final.to_excel(writer, sheet_name=list)
#
#     print('^' * 50)
