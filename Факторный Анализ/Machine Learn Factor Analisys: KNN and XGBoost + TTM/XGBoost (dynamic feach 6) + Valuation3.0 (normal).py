...
# -------------------------------- РАСЧЕТНЫЙ ЦИКЛ ----------------------------------------
    ...
    try:
        with pd.ExcelWriter('PORTFOLIO PER YEAR/PORTFOLIO_XGBoost_(dynamic_feach_6)_Valuation3.0_(normal).xlsx', mode='a') as writer:
            portfolio_df_final.to_excel(writer, sheet_name=list)
    except:
        with pd.ExcelWriter('PORTFOLIO PER YEAR/PORTFOLIO_XGBoost_(dynamic_feach_6)_Valuation3.0_(normal).xlsx', mode='w') as writer:
            portfolio_df_final.to_excel(writer, sheet_name=list)
